		...
		while(NullinaRow !=7){
			...
			if(jackid.equals("empty")) {
				...
				if(returnCables.size() == 1) {
					...
					String worktag = GrabWorktag(returnCablesX.get(0));
					...
					if(returnCables.size() == 1) {
						...
						manUpdate = true;
						System.out.println("2nd manual update " + jackid + " " + jackidRet.size() + " item nbrs found " + ItemNbr.size());
				...
				if(!manUpdate && !noResults) {
					for(int i = 0; i < ItemNbr.size();++i) {
						...
						else if(ItemNbr.get(i).contains("702")) {
